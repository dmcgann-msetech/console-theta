...
- [ ] **Documents:** the page renders as collapsible folders (Invoices, Paid Receipts, Quotes, Work Orders, Service Requests, Change Orders, Proposals, Consultations, Diagnostics, Sign-offs, Terms, Intake, Photos / Images, Files / Other Documents) instead of a single mixed table. Saved invoices show under **Invoices**; saved receipts show under **Paid Receipts**; an uploaded photo lands under **Photos / Images**; a non-image attachment (PDF, doc, etc.) lands under **Files / Other Documents**. View / Edit / Regen / Attach / Delete buttons are present on each row. Search filters across all folders; picking a type shows only that folder. View a saved invoice/receipt — print preview renders correctly on desktop and iOS.
